...
int PatchRegion(char* path, u32 len, const char* fmt, const char* mode) {
    ...
    return snprintf(path, len, fmt, mode, System::sInstance->GetInfo().GetWiilinkRegion());
}
...
static int GetFriendsSearchType(int curType, u32 regionId) {
    ...
    if (System::sInstance->GetInfo().GetWiilinkRegion() != friendRegionId) return curType;
...
static u32 PatchRKNetControllerRegion() {
    return System::sInstance->GetInfo().GetWiilinkRegion();
}
